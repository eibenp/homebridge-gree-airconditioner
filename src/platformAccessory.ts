...
import type { GreeACPlatform, MyPlatformAccessory } from "./platform"
import {
  PLATFORM_NAME,
  PLUGIN_NAME,
  type DeviceConfig,
  TEMPERATURE_TABLE,
  MODIFY_VERTICAL_SWING_POSITION,
  TS_TYPE,
  BINDING_TIMEOUT,
  TEMPERATURE_LIMITS,
...
export class GreeAirConditioner {
  ...
  constructor(
    ...
    if (this.platform.ports.indexOf(this.deviceConfig.port || 0) >= 0) {
      this.platform.log.warn(
        `[${this.getDeviceLabel()}] Warning: Configured port (%i) is already used - replacing with auto assigned port`,
    ...
    this.socket.bind(this.deviceConfig.port, undefined, () => {
      this.platform.log.info(
        `[${this.getDeviceLabel()}] Device handler is listening on UDP port %d`,
  ...
  initCharacteristics() {
    ...
    this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.CoolingThresholdTemperature).setProps({
    ...
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] CoolingThresholdTemperature - minValue: %s, maxValue: %s, minStep: %s`,
    ...
    // this is not a problem, this is information only that GREE is more restricitive than Apple's default
    this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.HeatingThresholdTemperature).setProps({
      minStep: this.deviceConfig.temperatureStepSize,
    ...
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] HeatingThresholdTemperature - minValue: %s, maxValue: %s, minStep: %s`,
      this.HeaterCooler?.getCharacteristic(
    ...
    this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.RotationSpeed).setProps({
      minValue: 0,
      maxValue: this.deviceConfig.speedSteps + 3,
      minStep: 1,
    })
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] RotationSpeed - minValue: %s, maxValue: %s, minStep: %s`,
  ...
  initAccessory() {
    ...
    if (!this.accessory.registered) {
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] Registering new accessory in homebridge:`,
      ...
      this.accessory
        ...
        .setCharacteristic(
          ...
          this.accessory.context.device.ver
            ? this.accessory.context.device.ver.substring(this.accessory.context.device.ver.lastIndexOf("V") + 1)
            : "1.0.0",
        )
    ...
    if (this.tsAccessoryMac) {
      this.tsAccessory = new GreeAirConditionerTS(
        this.platform,
        this.platform.getAccessory(this.accessory.context.device.mac + "_ts"),
      )
    }
    ...
    if (this.accessory.context.HeaterCoolerRotationSpeed === undefined) {
      this.accessory.context.HeaterCoolerRotationSpeed =
        ...
        this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.RotationSpeed).props.minValue ||
        0
    }
    ...
    this.accessory
      ...
      .setCharacteristic(
        ...
        this.accessory.context.device.hid &&
          this.accessory.context.device.hid.lastIndexOf("V") >= 0 &&
          this.accessory.context.device.hid.lastIndexOf("V") < this.accessory.context.device.hid.lastIndexOf(".")
          ? this.accessory.context.device.hid.substring(
              this.accessory.context.device.hid.lastIndexOf("V") + 1,
              this.accessory.context.device.hid.lastIndexOf("."),
            )
          : "1.0.0",
      )
    ...
    if (this.deviceConfig.temperatureSensor === TS_TYPE.child) {
      if (tss) {
        ...
        this.TemperatureSensor = this.accessory.addService(
          this.platform.Service.TemperatureSensor,
          "Temperature Sensor " + this.HeaterCooler.displayName,
        )
      }
    } else {
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] Temperature Sensor child service not allowed`,
        tss?.displayName !== undefined ? "(" + tss?.displayName + ")" : "",
      )
      if (tss !== undefined) {
        this.platform.log.debug(
          `[${this.getDeviceLabel()}] Remove Temperature Sensor child service (%s)`,
          tss.displayName,
        )
    ...
    if (this.deviceConfig.fanControlEnabled) {
      if (fs) {
        ...
        this.Fan = this.accessory.addService(this.platform.Service.Fanv2, "Fan " + this.HeaterCooler.displayName)
        // set static characeristics
        this.Fan?.getCharacteristic(this.platform.Characteristic.RotationSpeed).setProps({
          minValue: 0,
          maxValue: 100,
          minStep: 100 / (this.deviceConfig.speedSteps + 1),
        })
        this.platform.log.debug(
          `[${this.getDeviceLabel()}] Fan RotationSpeed - minValue: %s, maxValue: %s, minStep: %s`,
      ...
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] Fan child service not allowed`,
    ...
    this.HeaterCooler.getCharacteristic(this.platform.Characteristic.CurrentHeaterCoolerState).onGet(
      this.getCurrentHeaterCoolerState.bind(this),
    )

    // register handlers for the Current Fan State Characteristic
    this.Fan?.getCharacteristic(this.platform.Characteristic.CurrentFanState).onGet(this.getCurrentFanState.bind(this))
    ...
    this.HeaterCooler.getCharacteristic(this.platform.Characteristic.CurrentTemperature).onGet(
      this.getCurrentTemperature.bind(this, "Heater-Cooler"),
    )
    this.TemperatureSensor?.getCharacteristic(this.platform.Characteristic.CurrentTemperature).onGet(
      this.getCurrentTemperature.bind(this, "Temperature Sensor"),
    )
      ...
      switch (bindNo) {
        ...
        default: {
          this.platform.log.error(
            `[${this.getDeviceLabel()}] Error: Device is not bound`,
  ...
  async setActive(value: CharacteristicValue) {
    ...
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] Set Heater-Cooler Active ->`,
      powerValue ? "ACTIVE" : "INACTIVE",
    )
    this.power = powerValue
    if (
      powerValue &&
      this.Fan?.getCharacteristic(this.platform.Characteristic.Active).value ===
        this.platform.Characteristic.Active.ACTIVE
    ) {
      this.Fan?.getCharacteristic(this.platform.Characteristic.Active).updateValue(
  ...
  async setFanActive(value: CharacteristicValue) {
    ...
    if (
      powerValue &&
      this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.Active).value ===
        this.platform.Characteristic.Active.ACTIVE
    ) {
      this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.Active).updateValue(
        this.platform.Characteristic.Active.INACTIVE,
      )
    }
  }
  ...
  async setTemperatureDisplayUnits(value: CharacteristicValue) {
    ...
    this.units =
      value === this.platform.Characteristic.TemperatureDisplayUnits.CELSIUS
        ? commands.units.value.celsius
        : commands.units.value.fahrenheit
  }
  ...
  async setSwingMode(value: CharacteristicValue) {
    ...
    this.swingMode =
      value === this.platform.Characteristic.SwingMode.SWING_ENABLED
        ...
        : [
              MODIFY_VERTICAL_SWING_POSITION.overrideDefPowerOnOscDisable,
              MODIFY_VERTICAL_SWING_POSITION.setPowerOnOscDisable,
            ].includes(
              this.deviceConfig.modifyVerticalSwingPosition || DEFAULT_DEVICE_CONFIG.modifyVerticalSwingPosition,
            )
          ? this.deviceConfig.defaultVerticalSwing || DEFAULT_DEVICE_CONFIG.defaultVerticalSwing
          : DEFAULT_DEVICE_CONFIG.defaultVerticalSwing
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] Set SwingMode ->`,
      logValue +
        (logValue === "DISABLED"
  ...
  async setFanSwingMode(value: CharacteristicValue) {
    ...
    this.swingMode =
      value === this.platform.Characteristic.SwingMode.SWING_ENABLED
        ? commands.swingVertical.value.full
        : this.deviceConfig.modifyVerticalSwingPosition === MODIFY_VERTICAL_SWING_POSITION.overrideDefPowerOnOscDisable
          ? this.deviceConfig.defaultVerticalSwing || DEFAULT_DEVICE_CONFIG.defaultVerticalSwing
          : this.deviceConfig.modifyVerticalSwingPosition === MODIFY_VERTICAL_SWING_POSITION.setPowerOnOscDisable
            ? this.deviceConfig.defaultFanVerticalSwing || DEFAULT_DEVICE_CONFIG.defaultFanVerticalSwing
            : DEFAULT_DEVICE_CONFIG.defaultFanVerticalSwing
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] Set Fan SwingMode ->`,
      logValue +
        (logValue === "DISABLED"
          ? ` (Position: ${this.getKeyName(commands.swingVertical.value, this.swingMode)})`
          : ""),
  ...
  async setRotationSpeed(value: CharacteristicValue) {
    const maxSpeed =
      this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.RotationSpeed).props.maxValue ||
    ...
    switch (value) {
      ...
      case 2: // auto
        this.platform.log.debug(
          `[${this.getDeviceLabel()}] Set Heater-Cooler RotationSpeed ->`,
          value + " (" + this.getKeyName(commands.speed.value, commands.speed.value.auto) + ")",
        )
      ...
      case 3: // low
        this.platform.log.debug(
          `[${this.getDeviceLabel()}] Set Heater-Cooler RotationSpeed ->`,
          value + " (" + this.getKeyName(commands.speed.value, commands.speed.value.low) + ")",
        )
      ...
      case 6: // mediumHigh / powerful
        if (maxSpeed === 8) {
          ...
          this.platform.log.debug(
            `[${this.getDeviceLabel()}] Set Heater-Cooler RotationSpeed ->`,
            value + " (" + this.getKeyName(commands.speed.value, commands.speed.value.mediumHigh) + ")",
          )
      ...
      case 7: // high
        this.platform.log.debug(
          `[${this.getDeviceLabel()}] Set Heater-Cooler RotationSpeed ->`,
          value + " (" + this.getKeyName(commands.speed.value, commands.speed.value.high) + ")",
        )
  ...
  async setFanRotationSpeed(value: CharacteristicValue) {
    if (
      ...
      this.Fan?.getCharacteristic(this.platform.Characteristic.Active).setValue(
        this.platform.Characteristic.Active.ACTIVE,
      )
    }
  ...
  */
  async getActive(): Promise<CharacteristicValue> {
    const currentPower =
      this.power && [commands.mode.value.cool, commands.mode.value.heat, commands.mode.value.auto].includes(this.mode)
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] Get Heater-Cooler Active ->`,
  ...
  async getTemperatureDisplayUnits(): Promise<CharacteristicValue> {
    const currentValue =
      this.units === commands.units.value.celsius
        ? this.platform.Characteristic.TemperatureDisplayUnits.CELSIUS
        : this.platform.Characteristic.TemperatureDisplayUnits.FAHRENHEIT
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] Get TemperatureDisplayUnits ->`,
  ...
  async getRotationSpeed(): Promise<CharacteristicValue> {
    ...
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] Get Heater-Cooler RotationSpeed ->`,
      value + " (" + logValue + ")",
    )
    return value
  }
  ...
  async getFanRotationSpeed(): Promise<CharacteristicValue> {
    const minStep =
      this.Fan?.getCharacteristic(this.platform.Characteristic.RotationSpeed).props.minStep ||
    ...
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] Get Fan RotationSpeed -> ${Math.round(value as number)}% (${logValueName})`,
    )
    return value
  }
  ...
  calcDeviceTargetTemp(temp: number, unit?: number): number {
    if (
      unit === commands.units.value.celsius ||
      (unit === undefined &&
        this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.TemperatureDisplayUnits).value ===
          this.platform.Characteristic.TemperatureDisplayUnits.CELSIUS)
    ) {
    ...
    const correction =
      (baseFahrenheitDecimalPart >= 0.05 && baseFahrenheitDecimalPart < 0.15) ||
  ...
  calcDeviceTargetOffset(temp: number, unit?: number): number {
    ...
    const offset =
      ...
      (baseFahrenheitDecimalPart >= 0.75 && baseFahrenheitDecimalPart < 0.85)
        ? 1
        : 0
  ...
  getTargetTempFromDevice(temp, offset, unit): number {
    ...
    const heatingTargetValue: number =
      (this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.HeatingThresholdTemperature).value as
        | number
        | undefined) || 25
    const coolingTargetValue: number =
      (this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.CoolingThresholdTemperature).value as
        | number
        | undefined) || 25
    switch (this.mode) {
      case commands.mode.value.heat:
    ...
    if (
      (targetValue === 12.5 && value === 13) ||
      (targetValue === 17.5 && value === 18) ||
  ...
  set power(value: boolean) {
    if (
      ...
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] power -> no change (${this.power}, ${this.powerPending}, ${this.mode},`,
        `${this.modePending})`,
      )
      return
    }
      ...
      switch (
        ...
        case this.platform.Characteristic.TargetHeaterCoolerState.COOL:
          if (this.mode !== commands.mode.value.cool) {
            ...
            if (
              this.deviceConfig.xFanEnabled &&
              (this.status[commands.xFan.code] || commands.xFan.value.off) !== commands.xFan.value.on
            ) {
        ...
        case this.platform.Characteristic.TargetHeaterCoolerState.HEAT:
          if (this.mode !== commands.mode.value.heat) {
            ...
            if (
              this.deviceConfig.xFanEnabled &&
              (this.status[commands.xFan.code] || commands.xFan.value.on) !== commands.xFan.value.off
            ) {
      ...
      if (powerValue === commands.power.value.on) {
        if (this.accessory.context.HeaterCoolerRotationSpeed !== 0) {
          ...
          switch (this.accessory.context.HeaterCoolerRotationSpeed) {
            ...
            case 2: // auto
              if (
                this.quietMode !== commands.quietMode.value.off ||
                this.powerfulMode !== commands.powerfulMode.value.off
              ) {
                command[commands.quietMode.code] = commands.quietMode.value.off
                logValue +=
                  (logValue ? ", " : "") +
        ...
        if (
          ([
            MODIFY_VERTICAL_SWING_POSITION.overrideDefPowerOnOscDisable,
            MODIFY_VERTICAL_SWING_POSITION.overrideDefPowerOn,
          ].includes(
            this.deviceConfig.modifyVerticalSwingPosition || DEFAULT_DEVICE_CONFIG.modifyVerticalSwingPosition,
          ) &&
            this.swingMode === commands.swingVertical.value.default) ||
          [MODIFY_VERTICAL_SWING_POSITION.setPowerOnOscDisable, MODIFY_VERTICAL_SWING_POSITION.setPowerOn].includes(
            this.deviceConfig.modifyVerticalSwingPosition || DEFAULT_DEVICE_CONFIG.modifyVerticalSwingPosition,
          )
        ) {
  ...
  set fanpower(value: boolean) {
    if (this.powerPending !== -1 || this.modePending !== -1) {
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] fanpower -> no change (${this.powerPending}, ${this.modePending})`,
      )
      return
    }
  ...
  set mode(value) {
    ...
    if (
      this.accessory.context.HeaterCoolerRotationSpeed !== 0 &&
      ...
      switch (this.accessory.context.HeaterCoolerRotationSpeed) {
        ...
        case 4: // mediumLow / medium
          ...
          logValue +=
            (logValue ? ", " : "") + "speed -> " + this.getKeyName(commands.speed.value, command[commands.speed.code])
          break
        case 5: // medium / high
          if (
            this.quietMode !== commands.quietMode.value.off ||
        ...
        case 6: // mediumHigh / powerful
          if (maxSpeed === 8) {
            ...
            logValue +=
              ...
              "quietMode -> " +
              this.getKeyName(commands.quietMode.value, commands.quietMode.value.off)
            command[commands.powerfulMode.code] =
              value === commands.mode.value.auto ? commands.powerfulMode.value.off : commands.powerfulMode.value.on
            logValue +=
              (logValue ? ", " : "") +
              "powerfulMode -> " +
              this.getKeyName(
                commands.powerfulMode.value,
                value === commands.mode.value.auto ? commands.powerfulMode.value.off : commands.powerfulMode.value.on,
              )
            command[commands.speed.code] = commands.speed.value.high
            logValue +=
  ...
  get targetTemperature() {
    ...
    switch (this.mode) {
      case commands.mode.value.cool:
        minValue = Math.max(
          ...
          this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.CoolingThresholdTemperature).props
            .minValue || 10,
        )
        maxValue = Math.min(
      ...
      case commands.mode.value.heat:
        minValue = Math.max(
          this.deviceConfig.minimumTargetTemperature,
          this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.HeatingThresholdTemperature).props
            .minValue || 0,
        )
        maxValue = Math.min(
          this.deviceConfig.maximumTargetTemperature,
    ...
    return Math.max(
      Math.min(
        this.getTargetTempFromDevice(
          this.status[commands.targetTemperature.code] || 25,
          this.status[commands.temperatureOffset.code] || 0,
          this.status[commands.units.code],
        ),
        maxValue,
      ),
      minValue,
  ...
  set targetTemperature(value) {
    ...
    const displayUnits = this.HeaterCooler?.getCharacteristic(
      this.platform.Characteristic.TemperatureDisplayUnits,
    ).value
    const deviceDisplayUnits =
  ...
  set units(value) {
    ...
    const actTemp = this.getTargetTempFromDevice(
      this.status[commands.targetTemperature.code],
      this.status[commands.temperatureOffset.code],
      this.units,
    )
  ...
  set swingMode(value) {
    ...
    this.platform.log.info(
      `[${this.getDeviceLabel()}] swingVertical ->`,
      this.getKeyName(commands.swingVertical.value, value),
    )
    this.sendCommand(command)
  }
  ...
  set speed(value) {
    if (
      ...
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] speed -> no change (${value}, ${this.quietMode}, ${this.powerfulMode})`,
      )
      return
    }
    ...
    command[commands.powerfulMode.code] = commands.powerfulMode.value.off
    this.platform.log.info(
      `[${this.getDeviceLabel()}] speed ->`,
      this.getKeyName(commands.speed.value, value) +
        ", quietMode -> " +
        this.getKeyName(commands.quietMode.value, commands.quietMode.value.off) +
        ", powerfulMode -> " +
        this.getKeyName(commands.powerfulMode.value, commands.powerfulMode.value.off),
  ...
  updateStatus(props: string[]) {
    ...
    if (props.includes(commands.power.code) || props.includes(commands.mode.code)) {
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] updateStatus (Heater-Cooler Active) ->`,
        hcActive ? "ACTIVE" : "INACTIVE",
      )
      this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.Active).updateValue(
        hcActive ? this.platform.Characteristic.Active.ACTIVE : this.platform.Characteristic.Active.INACTIVE,
      )
    }
    // Fan Active
    if (this.Fan && (props.includes(commands.power.code) || props.includes(commands.mode.code))) {
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] updateStatus (Fan Active) ->`,
        fanActive ? "ACTIVE" : "INACTIVE",
      )
      this.Fan?.getCharacteristic(this.platform.Characteristic.Active).updateValue(
        fanActive ? this.platform.Characteristic.Active.ACTIVE : this.platform.Characteristic.Active.INACTIVE,
      )
    }
      ...
      if (this.power) {
        switch (this.mode) {
          case commands.mode.value.cool:
            ...
            this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.CurrentHeaterCoolerState).updateValue(
          ...
          case commands.mode.value.auto:
            if (this.currentTemperature > this.targetTemperature + 1.5) {
              this.platform.log.debug(
                `[${this.getDeviceLabel()}] updateStatus (Current Heater-Cooler State) -> COOLING`,
              )
              this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.CurrentHeaterCoolerState).updateValue(
                this.platform.Characteristic.CurrentHeaterCoolerState.COOLING,
              )
              ...
              this.platform.log.debug(
                `[${this.getDeviceLabel()}] updateStatus (Current Heater-Cooler State) -> HEATING`,
              )
              this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.CurrentHeaterCoolerState).updateValue(
                this.platform.Characteristic.CurrentHeaterCoolerState.HEATING,
              )
    ...
    if (this.Fan && props.includes(commands.mode.code)) {
      if (this.power && this.mode === commands.mode.value.fan) {
        ...
        this.Fan?.getCharacteristic(this.platform.Characteristic.CurrentFanState).updateValue(
    ...
    if (
      props.includes(commands.mode.code) &&
      this.power &&
      [commands.mode.value.cool, commands.mode.value.heat, commands.mode.value.auto].includes(this.mode)
    ) {
      switch (this.mode) {
        case commands.mode.value.cool:
          ...
          this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.TargetHeaterCoolerState).updateValue(
            this.platform.Characteristic.TargetHeaterCoolerState.COOL,
          )
    ...
    if (props.includes(commands.temperature.code)) {
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] updateStatus (Current Temperature) ->`,
        this.currentTemperature,
      )
      this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.CurrentTemperature).updateValue(
        this.currentTemperature,
      )
      this.TemperatureSensor?.getCharacteristic(this.platform.Characteristic.CurrentTemperature).updateValue(
        this.currentTemperature,
      )
      this.tsAccessory?.setCurrentTemperature(this.currentTemperature)
      this.tsAccessory?.TemperatureSensor.getCharacteristic(
    ...
    if (
      props.includes(commands.targetTemperature.code) &&
      this.power &&
      (this.mode === commands.mode.value.cool || this.mode === commands.mode.value.auto)
    ) {
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] updateStatus (Cooling Threshold Temperature) ->`,
        this.targetTemperature,
      )
      this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.CoolingThresholdTemperature).updateValue(
        this.targetTemperature,
      )
    }
    ...
    if (
      ...
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] updateStatus (Heating Threshold Temperature) ->`,
        this.targetTemperature,
      )
      this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.HeatingThresholdTemperature).updateValue(
        this.targetTemperature,
      )
    }
    ...
    if (props.includes(commands.units.code)) {
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] updateStatus (Temperature Display Units) ->`,
        this.units === commands.units.value.celsius ? "CELSIUS" : "FAHRENHEIT",
      )
      this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.TemperatureDisplayUnits).updateValue(
        this.units === commands.units.value.celsius
          ? this.platform.Characteristic.TemperatureDisplayUnits.CELSIUS
          : this.platform.Characteristic.TemperatureDisplayUnits.FAHRENHEIT,
      )
    ...
    if (props.includes(commands.swingVertical.code) && this.power) {
      ...
      if (
        [
          commands.swingVertical.value.full,
          commands.swingVertical.value.swingHigher,
          commands.swingVertical.value.swingHighest,
          commands.swingVertical.value.swingLower,
          commands.swingVertical.value.swingLowest,
          commands.swingVertical.value.swingMiddle,
        ].includes(this.swingMode)
      ) {
      ...
      this.HeaterCooler?.getCharacteristic(this.platform.Characteristic.SwingMode).updateValue(swing)
    }
    ...
    if (hcActive) {
      ...
      if (props.includes(commands.quietMode.code) && this.quietMode === commands.quietMode.value.on) {
        ...
        // powerfulMode -> on
        this.platform.log.debug(
          `[${this.getDeviceLabel()}] updateStatus (Heater-Cooler Rotation Speed) ->`,
    ...
    if (fanActive) {
      ...
      const speedName = this.getKeyName(commands.speed.value, this.speed)
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] updateStatus (Fan Rotation Speed) ->`,
  ...
  handleMessage = (msg, rinfo) => {
    if (this.accessory.context.device.address === rinfo.address) {
      ...
      this.platform.log.debug(
        `[${this.getDeviceLabel()}] handleMessage -> Encryption version: %i`,
          ...
          if (this.accessory.bound) {
            ...
            if (!(pack.cols as [string]).includes(commands.temperature.code) || invalidTempFromDevice) {
              ...
              const targetTemp: number =
                this.status[commands.targetTemperature.code] !== undefined
                  ? this.status[commands.targetTemperature.code]
                  : 25 // use default if target temperature is also unknown
              this.status[commands.temperature.code] = targetTemp + this.deviceConfig.sensorOffset
              this.platform.log.debug(
                `[${this.getDeviceLabel()}] Current temperature not available`,
                "- Threshold temperature is used as current (" +
              ...
              if (this.TemperatureSensor !== undefined) {
                ...
                this.platform.log.debug(
                  `[${this.getDeviceLabel()}] temperature is not accessible -> Temperature Sensor removed`,
                )
              }
            }
  ...
  sendMessage(message) {
    ...
    this.platform.log.debug(
      `[${this.getDeviceLabel()}] sendMessage -> Encryption version: %i`,
      this.accessory.context.device.encryptionVersion,
    )
